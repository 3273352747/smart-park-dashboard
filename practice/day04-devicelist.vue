...
const filterDevices = computed(() =>{
    ...
    return deviceManager.value.filter((device) => {
        return device.status === activeStatus.value
    })
})
const totalDevices = computed(() => deviceManager.value.length)
const runningCount = computed(() => {
    return deviceManager.value.filter((device) => device.status === "运行中").length
})
const offlineCount = computed(() => {
    return deviceManager.value.filter((device) => device.status === "离线").length
})
const alarmingCount = computed(() => {
    return deviceManager.value.filter((device) => device.status === "告警中").length
})
</script>
...
<template>
    <main>
        ...
        <ul>
            <li v-for="device in filterDevices" :key="device.code">
        ...
        <p v-if="filterDevices.length === 0" class="empty-text">暂无符合条件的设备</p>
    </main>
</template>
